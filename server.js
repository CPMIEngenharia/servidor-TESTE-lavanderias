...
// --- 6. O CÉREBRO DE DESPACHO (COM FALLBACK PARA PLACA ANTIGA) ---
function executarDisparo(idMaquina, parametro) {
    ...
    if (!idMaquina.toLowerCase().includes('sec')) {
        ...
        // Fallback em 12s: se a placa antiga não respondeu, dispara CMD_SECAR
        setTimeout(() => {
            let st = STATUS_CACHE[idMaquina] || "DISPONIVEL";
            if (!st.includes("TEMPO:") && !st.includes("SECANDO") && !st.includes("OCUPADA") && !st.includes("LAVANDO")) {
                console.log(`⚠️ [PLACA ANTIGA EM ${idMaquina}] Disparando CMD_SECAR`);
...
// --- 14. WEBHOOK (USA O CÉREBRO, SEM FORÇAR OCUPADA) ---
